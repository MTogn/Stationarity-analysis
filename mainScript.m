...
for burstCtr = burstStartIndex:burstEndIndex;
    ...
    ITSStruc(1).isStationaryP95(burstCtr) = RATestWrapper(burstBeamVelocities,minNind,paramStruc);
...
for beamCtr = 1:4
    ...
    contourf(timeArr',HASBArr',ITSStruc(beamCtr).beamITS(burstStartIndex:burstEndIndex,1:length(plotParams.HASBVec))','LineStyle','none');
...
for beamCtr = 1:4
    ...
    velITSScatterAxHand(beamCtr).Title.String = strcat("Beam ",int2str(beamCtr));
    velITSScatterAxHand(beamCtr).FontSize = 14;
end
%Unify the limits of the y-scatter
scatterYLim = get(velITSScatterAxHand(1),'YLim'); scatterYLimLo = scatterYLim(1); scatterYLimHi = scatterYLim(2);
for beamCtr = 2:4
    scatterYLim = get(velITSScatterAxHand(beamCtr),'YLim');
...
for beamCtr = 1:4
    set(velITSScatterAxHand(beamCtr),'YLim',scatterYLim);
end
...
%Here we compare ITS with wave height, assuming that it's already been
...
for beamCtr = 1:4
    ...
    waveITSScatterAxHand(beamCtr).Title.String = strcat("Beam ",int2str(beamCtr));
    waveITSScatterAxHand(beamCtr).FontSize = 14;
end
%Unify the limits of the y-scatter
scatterYLim = get(waveITSScatterAxHand(1),'YLim'); scatterYLimLo = scatterYLim(1); scatterYLimHi = scatterYLim(2);
for beamCtr = 2:4
    scatterYLim = get(waveITSScatterAxHand(beamCtr),'YLim');
    if scatterYLim(1) < scatterYLimLo, scatterYLimLo = scatterYLim(1); end
    if scatterYLim(2) > scatterYLimHi, scatterYLimHi = scatterYLim(2); end
end
scatterYLim = [scatterYLimLo, scatterYLimHi];
for beamCtr = 1:4
    set(waveITSScatterAxHand(beamCtr),'YLim',scatterYLim);
end
